...
struct GGUFParser {
    func parseHeader(at url: URL) throws -> GGUFMetadata {
        ...
        for _ in 0..<metadataCount {
            ...
            case "general.parameter_count":
                parameterCount = Int64(try readNumericValue(file, type: valueType))
            case "general.architecture":
                architecture = try readStringValue(file, type: valueType)
            case "general.name":
                name = try readStringValue(file, type: valueType)
            case "general.file_type":
                fileType = Int32(try readNumericValue(file, type: valueType))
            default:
                try skipValue(file, type: valueType)
            }

            if parameterCount != nil, architecture != nil, name != nil, fileType != nil {
                break
            }
        }
    ...
    private func readString(_ file: FileHandle) throws -> String {
        ...
        let data = try read(file, count: Int(length))
        guard let value = String(data: data, encoding: .utf8) else { throw GGUFParserError.malformed }
        return value
    }

    private func readStringValue(_ file: FileHandle, type: UInt32) throws -> String {
        guard type == 8 else {
            try skipValue(file, type: type)
            throw GGUFParserError.malformed
        }
    ...
    private func readNumericValue(_ file: FileHandle, type: UInt32) throws -> Int64 {
        ...
        case 11: return try read(file) as Int64
        default:
            try skipValue(file, type: type)
            throw GGUFParserError.malformed
        }
    ...
    private func skipValue(_ file: FileHandle, type: UInt32) throws {
        let primitiveSizes: [UInt32: Int] = [0: 1, 1: 1, 2: 2, 3: 2, 4: 4, 5: 4, 6: 4, 7: 1, 10: 8, 11: 8]
        ...
        guard let size = primitiveSizes[type] else {
            throw GGUFParserError.malformed
        }
        _ = try read(file, count: size)
    }
}
